...
| `api_key_header` | Optional HTTP header name for sending the `token` to a gateway (for example, `LUNAROUTE-API-KEY`). The token is sent verbatim in this header in addition to the provider's normal auth | No | - |
